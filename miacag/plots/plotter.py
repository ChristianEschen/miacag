...
def getNormConfMat(df, labels_col, preds_col,
                   plot_name, f1, output, num_classes):
...
def plot_results(sql_config, output_plots, num_classes, roc=False):
    ...
    for label_name in sql_config['labels_names']:
        ...
        df[label_name + '_predictions'] = \
            df[label_name + '_predictions'].astype(float).astype(int)
...
def plot_roc_curve(labels, confidences, output_plots, plot_name):
