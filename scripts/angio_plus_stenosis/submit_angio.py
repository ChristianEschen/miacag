...
if __name__ == '__main__':
      ...
      #  if torch.distributed.get_rank() == 0:
        ...
        if checkExpExists(config_path[i], config['output']) is False:
            ...
            if i == 0:
                ...
                output_csv_test_sten = os.path.join(config['output'], 'results.csv')

                df_results = create_empty_csv(output_csv_test)
                df_results_sten = create_empty_csv(output_csv_test_sten)
            ...
            create_empty_csv(output_csv_test_sten)
            ...
            output_plots_train = os.path.join(output_plots, 'train_ca_class')
            output_plots_val = os.path.join(output_plots, 'val_ca_class')
            output_plots_test = os.path.join(output_plots, 'test_ca_class')

            output_plots_train_sten = os.path.join(output_plots, 'train_sten')
            output_plots_val_sten = os.path.join(output_plots, 'val_sten')
            output_plots_test_sten = os.path.join(output_plots, 'test_sten')

            mkFolder(output_plots_train)
            mkFolder(output_plots_test)
            mkFolder(output_plots_val)

            mkFolder(output_plots_train_sten)
            mkFolder(output_plots_test_sten)
            mkFolder(output_plots_val_sten)
        ...
        #   config['qeury'] = config['query_']
            ...
            if torch.distributed.get_rank() == 0:
                ...
                csv_results_sten = appendDataFrame(sql_config={
                                    'labels_names': config['labels_names'],
                                    'database': config['database'],
                                    'username': config['username'],
                                    'password': config['password'],
    ...
    if torch.distributed.get_rank() == 0:
        ...
        csv_results_sten = pd.DataFrame(csv_results_sten)
        csv_results_sten.to_csv(output_csv_test_sten, index=False, header=True)
