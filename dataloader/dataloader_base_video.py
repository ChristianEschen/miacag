...
class VideoDataloaderBase(data.Dataset):
    def __init__(self, video_path, csv_path_file, transform=None):
        super(VideoDataloaderBase, self).__init__()
        self.video_path = video_path
        self.csv = pd.read_csv(csv_path_file)
        self.labels = self.csv['label']
        self.mapped_labels = self.map_labels(self.labels)
    ...
    def map_labels(self, labels):
        ...
        mapped_labels = pd.Series([mapping[k]
...
class VideoDataloaderTest(VideoDataloaderBase):
    def __init__(self, video_path, csv_path, transform=None):
        super(VideoDataloaderTest, self).__init__(video_path,
                                                  csv_path,
                                                  transform)
    ...
    def create_patches(x, nr_frames, transform):
        ...
        x = VideoDataloaderTest.pad_volume(x, pad_size)
        crop_size = VideoDataloaderTest.get_crop_size(transform)
...
class VideoDataloaderTrain(VideoDataloaderBase):
    def __init__(self, video_path, csv_path, transform=None):
        super(VideoDataloaderTrain, self).__init__(video_path,
                                                   csv_path,
                                                   transform)
        self.class_counts = self.mapped_labels.value_counts().to_list()
        self.class_weights = [self.num_samples/self.class_counts[i] for
                              i in range(len(self.class_counts))]  # [::-1]
        self.weights = [self.class_weights[self.mapped_labels[i]]
                        for i in range(int(self.num_samples))]
        self.sampler = WeightedRandomSampler(torch.DoubleTensor(self.weights),
