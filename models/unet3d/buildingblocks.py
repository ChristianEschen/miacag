...
from models.blocks_3D import SingleConv, DoubleConv


class ExtResNetBlock(nn.Module):
    """
    Basic UNet block consisting of a SingleConv followed by the residual block.
    The SingleConv takes care of increasing/decreasing the number
    of channels and also ensures that the number
    of output channels is compatible with the residual block that follows.
    This block can be used instead of
    standard DoubleConv in the Encoder module.
    Motivated by: https://arxiv.org/pdf/1706.00120.pdf
    Notice we use ELU instead of ReLU (order='cge'
    and put non-linearity after the groupnorm.
    """
    ...
    def __init__(self, in_channels, out_channels,
        ...
        self.conv2 = SingleConv(out_channels, out_channels,
                                ...
                                order=order, num_groups=num_groups)
        # remove non-linearity from the 3rd convolution since
        # it's going to be applied after adding the residual
        n_order = order
        for c in 'rel':
...
class Encoder(nn.Module):
    ...
    Args:
        ...
        padding (int or tuple): add zero-padding added
        to all three sides of the input
    """

    def __init__(self, in_channels, out_channels, conv_kernel_size=3,
                 apply_pooling=True,
                 pool_kernel_size=2, pool_type='max',
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3,
                 ...
                 conv_layer_order='gcr', num_groups=8,
                 mode='nearest', padding=1):
        super(Decoder, self).__init__()
        if basic_module == DoubleConv:
            # if DoubleConv is the basic_module use
...
class Upsampling(nn.Module):
    """
    Upsamples a given multi-channel 3D data using either
    ...
    def __init__(self, transposed_conv,
                 in_channels=None,
                 out_channels=None,
                 kernel_size=3,
