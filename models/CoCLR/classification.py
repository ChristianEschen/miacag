...
sys.path.append('../')
from models.CoCLR.S3 import S3D
...
class LinearClassifier(nn.Module):
    def __init__(self, 
        ...
        message = 'Classifier to %d classes with %s backbone;'\
            % (classes, network)
      ...
      #  self.backbone, self.param = select_backbone(network)
        ...
        if use_dropout:
            self.final_fc = nn.Sequential(
                nn.Dropout(dropout),
                nn.Linear(self.param['feature_size'], self.classes))
        else:
            self.final_fc = nn.Sequential(
                nn.Linear(self.param['feature_size'], self.classes))
